...
def extend_proj_set_with_next_wyckoff_position(calc, proj_set:ProjectionsSet):
    ...
    mult_wp = list(zip([m.multiplicity for m in group.Wyckoff_positions],[WyckoffPosition(position_str=wp, spacegroup=SpaceGroup.from_gpaw(calc)) for wp in wps])) #multiplcity is in the conventional cell, but this is not a problem as we are interested in the ordering
    ...
    return extended_proj_set,num_new_wf #return the extended projection set and the multiplicity of the added wyckoff position



def extend_to_energy_window(calc,energies,dos_total,K,nwann, proj_set:ProjectionsSet, outer_win:tuple, frozen_win:tuple,objective_window:tuple,max_iter=10) -> ProjectionsSet:
    '''
    ...
    while keep_going:
        ...
        if frozen_win[0] <= objective_window[0] and frozen_win[1] >= objective_window[1]:
            ...
            warnings.warn(f"Warning: Maximum number of iterations ({max_iter}) reached while trying to extend the projection set to cover the objective window. Objective window was not reached. Current frozen window: {frozen_win}, Objective window: {objective_window}.")
            keep_going = False
        else:
            ...
            new_nwann += 1*multiplicity#s orbitals, need to be multiplied by the multiplicity of the wyckoff position
            #adapt the outer window maximum to the new number of wannier functions
            emax_refined = find_emax_from_dos(energies=energies,dos_total=dos_total, n_wann=new_nwann, emin=outer_win[0],K=K)
            new_outer_win = (outer_win[0], emax_refined)
            #safety check the new windows
            outer_win, frozen_win = safety_check_windows(calc,nwann=new_nwann,outer_win=new_outer_win,frozen_win=outer_win)
