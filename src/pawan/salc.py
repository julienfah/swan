        ...
        for j in range(i + 1, len(blocks)):
            if not used[j] and blocks[j].shape[0] == Bi.shape[0] \
               and np.allclose(chars[i], chars[j], atol=1e-4):
                grp.append(blocks[j])
                used[j] = True
        comps.append((np.vstack(grp), Bi.shape[0], len(grp)))
    return comps
...
def hybrids_from_bonds(bonds, ops, shells, seed=0):
    ...
    import itertools
    best = max((float(np.einsum('jm,jm->', Sih @ sum(e * C for e, C in zip(eps, chans)), Y)),
...
def build(atoms, shells, cutoff=None, prefix="", symprec=1e-4, seed=0,
    ...
    shells : {element: 'spd' | [0,1,2]}. Elements absent get no projections.
    ...
    for i in sorted(set(ds.equivalent_atoms)):
        ...
        if cut is not False:          # False disables bond hybrids for this run
            try:
                ...
                if len(nb):
                    Tc, uniq = hybrids_from_bonds(nb, ops, sh, seed=seed)
                    if uniq:
                        T = Tc
