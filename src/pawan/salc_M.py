...
def _itertools_product_ns(shells, cand):
    """All same-index combinations of bound channels, for n_select='sum'."""
    import itertools as _it
    return _it.product(*[[n for n, _ in sorted(cand[l])] for l in shells])
...
def projectability_from_gpaw(calc, window, spin=0, atol=0.25,
    ...
    def weight_fn(iatom, shells, ops):
        ...
        if n_select == 'sum':
            dim = sum(2 * l + 1 for l in shells)
            M = _np.zeros((dim, dim))
            for ns in _itertools_product_ns(shells, cand):
                idx = _np.concatenate([_np.arange(slc[(l, n)].start,
                                                  slc[(l, n)].stop)
                                       for l, n in zip(shells, ns)])
                M += Mall[_np.ix_(idx, idx)]
        else:
            idx = _np.concatenate([_np.arange(slc[(l, chosen[l])].start,
                                              slc[(l, chosen[l])].stop)
                                   for l in shells])
            M = Mall[_np.ix_(idx, idx)]
