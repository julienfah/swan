...
def parse_position(position):
    """Fractional coordinates from floats, ints, or strings -- including
    fraction strings like '1/3' or sympy's exact-decimal form
    '333333333333333/1000000000000000' (an sympy Rational approximating 1/3,
    stringified). str(x) does NOT parse with float(): float() accepts plain
    decimals only, never a/b. Route every non-numeric entry through
    fractions.Fraction, which parses both 'a/b' and plain decimal strings, then
    reduces the sympy giant-fraction case back to something exact-ish via
    limit_denominator (sympy's decimal Rational is only an approximation of 1/3
    to begin with, so exact conversion is not the point -- getting back to 1/3
    rather than carrying 15 spurious digits is).
    """
