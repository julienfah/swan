...
def _aligned_residuals(dft_eig_energies, k_dft, wannier_interpolated_energies, k_wannier, outer_win, per_nk=False):
    """
    Align DFT and Wannier-interpolated eigenvalues onto common k-points (matching
    exactly if one k-grid is included in the other, interpolating otherwise), and
    return (residuals, fmask) restricted to bands/states inside outer_win.
    """
...
def least_square_deviation(dft_eig_energies, k_dft, wannier_interpolated_energies, k_wannier, outer_win,
    ...
    """
    residuals, fmask = _aligned_residuals(dft_eig_energies, k_dft, wannier_interpolated_energies, k_wannier,
                                           outer_win, per_nk=per_nk)
...
def max_deviation_within_frozen(dft_eig_energies, k_dft, wannier_interpolated_energies, k_wannier, frozen_win,
                  per_nk=True, scale=1000,top_pad=0.1):
    """
    Maximum absolute deviation between DFT and Wannier-interpolated eigenvalues,
    among (band,k) states inside outer_win. scale=1000 converts eV -> meV.
    """
    residuals, fmask = _aligned_residuals(dft_eig_energies, k_dft, wannier_interpolated_energies, k_wannier,
                                           (frozen_win[0], frozen_win[1] - top_pad), per_nk=per_nk)
    if not fmask.any():
        raise ValueError("no (band,k) states fall inside the window.")
    return scale * np.max(np.abs(residuals[fmask]))
...
def least_square_deviation_within_frozen(dft_eig_energies,k_dft, wannier_interpolated_energies,k_wannier,outer_win,frozen_win,
                                         per_nk=True, root=True, scale=1000, top_pad=0.1):
    """
    ...
    return least_square_deviation(dft_aligned, k_dft, wannier_interpolated_energies, k_wannier, (frozen_win[0], frozen_win[1] - top_pad),
